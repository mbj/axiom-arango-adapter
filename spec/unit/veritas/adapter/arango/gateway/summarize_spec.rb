...
describe Veritas::Adapter::Arango::Gateway, '#summarize' do
  ...
  context 'with no arguments' do
    ...
    it 'forwards the default summarize_with relation to relation#summarize' do
      relation.should_receive(:summarize) do |other|
        other.should equal(Veritas::TABLE_DEE)
      end
      subject
    end
  ...
  context 'with a relation' do
    ...
    it 'passes the context header into the context' do
      Veritas::Evaluator::Context.should_receive(:new).with(context_header)
      subject
    end
    ...
    it 'initializes the summarization with the gateway, the relation and the functions' do
      Veritas::Algebra::Summarization.should_receive(:new).with(object, summarize_with, functions)
      subject
    end
  end
end
