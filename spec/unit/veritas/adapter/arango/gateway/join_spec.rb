...
describe Veritas::Adapter::Arango::Gateway, '#join' do
  ...
  let(:operation)       { :join                                  }
  let(:factory)         { Veritas::Algebra::Join                 }
  ...
  context 'when passed a block' do
    ...
    before do
      Veritas::Algebra::Join.stub!(:new).with(relation, other_relation).and_return(join)
    end

    it { should equal(gateway) }

    it 'passes the relations to the join constructor' do
      Veritas::Algebra::Join.should_receive(:new).with(relation, other_relation)
      subject
    end
